test_that("merge_episodes works on a small set of values", {
  ...
  out1a <- dat1 |>
    ...
    dplyr::group_by(id) |>
    dplyr::summarise(
      interval = ivs::iv_groups(interval,
        abutting = FALSE
      ),
      .groups = "drop"
    )
...
test_that("merge_episodes works on a large set of values", {
  ...
  start2 <- as.Date("2020-01-01") + sample.int(365,
    size = n * 5,
    replace = TRUE
  )
  ...
  # POSIXct
  dat4 <- dplyr::tibble(
    id = id2, start = as.POSIXct(start2),
    end = as.POSIXct(end2)
  )
